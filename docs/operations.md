...
The release command keeps four-column label sidecars, stages one resolved EEA
group at a time, and reuses each downloaded source geometry shard across group
passes. This limits peak reference-data memory and disk usage. It requires a
valid `HF_TOKEN` with write access to the target repositories:
...
4. Process each geometry shard through every resolved EUNIS reference group
   while reusing the staged source shard; for Wikidata, process the matching
   link shard and collect bounded label counts and map bins at the same time.
