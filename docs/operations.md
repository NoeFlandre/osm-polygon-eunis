...
The release command keeps four-column label sidecars, stages bounded EEA
reference batches, and reuses each downloaded source geometry shard across
...
access to the target repositories. Each batch uses four bounded worker
...
4. Process each geometry shard through each bounded EUNIS reference batch while
   reusing the staged source shard; for Wikidata, process the matching link
   shard and collect bounded label counts and map bins at the same time.
